 ...
 */
export type AIPromptContentSimple =
    | string
    | AudioBuffer
    | ArrayBufferView
    | ArrayBuffer
 ...
 *
 * @param value - The content value to infer the type from.
 * @returns The inferred content type ('text', 'audio', or 'image').
 * @internal
 *
 * @example
 * ```ts
 * const type = inferContentType('Hello world'); // 'text'
 * const type = inferContentType(new ArrayBuffer(10)); // 'audio'
 * ```
 */
 ...
 *
 * @param content - The content (single item or array) to normalize.
 * @returns An array of internal content objects with inferred types.
 * @internal
 *
 * @example
 * ```ts
 ...
 *
 * const normalized = normalizeContent(['Hello', new ArrayBuffer(10)]);
 * // [{ type: 'text', value: 'Hello' }, { type: 'audio', value: ArrayBuffer }]
 * ```
 */
function normalizeContent(
    content: AIPromptContentSimple | AIPromptContentSimple[],
): AIPromptContentInternal[] {
...
export function useAIPrompt(
    ...
    const prompt = useCallback(
        async (input: string | AIPromptMessage[]) => {
            if (
                status === 'prompting' ||
                status === 'initializing' ||
                status === 'downloading'
            ...
            try {
                ...
                if (streaming) {
                    ...
                    const result = await session.prompt(normalizedInput, {
                        signal: abortControllerRef.current.signal,
                    })
                    setData(result)
                    setContextUsage(session.contextUsage || 0)
                ...
                setError(
                    err instanceof Error
                        ? err
                        : new Error('Unknown error during prompting'),
                )
                setStatus('error')
            }
        },
    ...
    const append = useCallback(
        async (messages: AIPromptMessage[]) => {
            try {
                const session = await createSession()
                ...
                setError(
                    err instanceof Error
                        ? err
                        : new Error('Unknown error during append'),
                )
                setStatus('error')
            }
        },
