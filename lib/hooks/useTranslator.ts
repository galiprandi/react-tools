...
export function useTranslator(
    ...
    const resolveLanguages = useCallback(
        async (
            textToDetect?: string,
        ): Promise<{
            ...
            let resolvedSource: SupportedLanguage = 'en'
            let resolvedTarget: SupportedLanguage = 'en'
                ...
                if (!textToDetect) {
                    ...
                    try {
                        if (
                            ...
                            // Security: check if constructor is valid
                            if (
                                ...
                                typeof LanguageDetector.availability ===
                                'function'
                            ) {
                                const avail =
                                ...
                                if (
                                    isActive &&
                                    avail !== 'unavailable' &&
                                    typeof LanguageDetector.create ===
                                        'function'
                                ) {
                                    const detector =
                                    ...
                                    if (results.length > 0) {
                                        const detected =
                                            results[0].detectedLanguage.split(
                                                '-',
        ...
        async (source: SupportedLanguage, target: SupportedLanguage) => {
            ...
            if (typeof Translator.availability === 'function') {
                ...
                if (avail === 'unavailable') {
                    throw new Error(
                        `Translator is not available for ${source} → ${target}`,
                    )
                }
    ...
    const translate = useCallback(
        async (textToTranslate: string) => {
            ...
            try {
                const { source, target } =
                    await resolveLanguages(textToTranslate)
                ...
                if (streaming) {
                    ...
                    for await (const chunk of stream) {
                        // The Translator API returns cumulative chunks, replace state to avoid DoS
                        setData(chunk)
                    }
                    setStatus('success')
                } else {
                ...
                setError(
                    err instanceof Error
                        ? err
                        : new Error('Unknown error during translation'),
                )
                setStatus('error')
            }
        },
        [streaming, resolveLanguages, createTranslator, status],
    )
    ...
    useEffect(() => {
        if (warmup) {
            resolveLanguages()
                .then(({ source, target }) => {
                    createTranslator(source, target)
                        .then(() => setStatus('idle'))
                        .catch(() => {})
                })
                .catch(() => {})
